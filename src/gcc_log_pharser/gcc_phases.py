...
def create_parser():
    ...
    parser.add_argument(
        "--save_path",
        help="Path to save file (otherwise console)",
        type=str,
        required=False,
    )
...
def main():
    global verbose, args, result_thread

    parser = create_parser()
    args = parser.parse_args()

    verbose = print if args.v else noprint
    result_thread = print if not args.save_path else RoiFile(args.save_path)
    ...
    if not path2:
        ...
        if (getarg('trace_format')):
            print_json(units, result_thread)
        else:
            print_units(units)
    else:
        units = collect_units(path)
...
def collect_units(path):
    ...
    def numgroup(index):
        return float(m.group(index))

    def strgroup(index):
        return m.group(index).strip()
...
def print_units(units, units2=None):
    ...
    def print_unit(unit, index=None):
        ...
        result_thread(unit.path if index is None else "{} : {}".format(index, unit.path))
        phases = unit.phases
        ...
        for k in sorted_phases_names:
            result_thread(phase_str(k, phases[k], index == None))
        result_thread("  TOTAL : {}".format(time_str(unit.wall_total)), '\n')

    def print_unit_diff(unit, unit2, diff_unit, index=None):
        global result_thread

        result_thread(diff_unit.path if index is None else "{} : {}".format(index, diff_unit.path))
        ...
        sorted_phases_names = sorted(diff_phases.keys(),
                                     ...
                                     reverse=phase_sort_order_reversed())
        for k in sorted_phases_names:
            result_thread(phase_diff_str(k, phases.get(k, None), phases2.get(k, None), index == None))
        result_thread("  TOTAL : {}".format(time_diff_str(
